...
from ..core_logic import (
    ...
    partitura_xml_keys, save_partitura_to_show_xml
)
...
from ..text_normalizer import normalize_russian_text
...
def build_project_data(project_dir: Path, project_name: str) -> dict:
    ...
    xml_keys = partitura_xml_keys(xml_path)
    return {
        "title": base_name,
        ...
        "partitura": [
            {
                ...
                "info": r.info,
                "command": r.command,
                "_xml_key": xml_keys[index] if index < len(xml_keys) else ""
            }
            for index, r in enumerate(part_rows)
        ]
    }

def ensure_partitura_xml_keys(data: dict, project_dir: Path) -> None:
    xml_path = find_project_xml(project_dir, data.get("title", ""))
    if xml_path is None:
        return
    rows = data.get("partitura", [])
    if not rows:
        return
    keys = partitura_xml_keys(xml_path)
    changed = False
    for index, row in enumerate(rows):
        if row.get("_xml_key"):
            continue
        if index < len(keys):
            row["_xml_key"] = keys[index]
            changed = True
    if changed:
...
def load_project_data(project_dir: Path, project_name: str) -> dict:
    ...
    if json_path.exists() and json_path.stat().st_size > 0:
        ...
        ensure_partitura_xml_keys(data, project_dir)
        write_project_json(project_dir, data)
        return data
...
PARTITURA_ROW_FIELDS = ("number", "name", "fade", "downfade", "delay", "trigger", "trigger_time", "info", "command")

def partitura_row_from_dict(row: dict, *, use_original: bool = False) -> PartituraRow:
    source = dict(row)
    if use_original and row.get("_translation_original"):
        original = row["_translation_original"]
        source["name"] = original.get("name", source.get("name", ""))
        source["info"] = original.get("info", source.get("info", ""))
    return PartituraRow(**{field: str(source.get(field, "") or "") for field in PARTITURA_ROW_FIELDS})
...
async def create_project(request: Request, title: str = Form(...), xml_file: UploadFile = File(...), db: Session = Depends(database.get_db)):
    ...
    project_data = {
        ...
        "partitura": [{"number": r.number, "name": r.name, "fade": r.fade, "downfade": r.downfade, "delay": r.delay, "trigger": r.trigger, "trigger_time": r.trigger_time, "info": r.info, "command": r.command, "_xml_key": xml_keys[index] if index < len(xml_keys) else ""} for index, r in enumerate(part_rows)]
    }
...
async def save_to_cloud_manual(project_name: str, request: Request, db: Session = Depends(database.get_db)):
    ...
    # 4. ALWAYS sync generated documents and show-file copies to cloud
    for f in project_dir.iterdir():
        if f.suffix in [".pdf", ".xlsx"] or f.name.endswith("_new.xml"):
...
async def local_files(project_name: str, request: Request):
    ...
    files = [
        ...
        if f.is_file() and (f.suffix in [".pdf", ".xlsx"] or f.name.endswith("_new.xml"))
    ]
    return sorted(files)
...
@router.post("/{project_name}/save_show_xml")
